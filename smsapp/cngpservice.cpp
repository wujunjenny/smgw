...
CCngpService::CCngpService(HANDLE hDrvObject, UL Type,UL InterfaceID) \
: CService(hDrvObject, Type, InterfaceID)
{
    ...
    //SetState(IF_BROKEN); move it to head
	
	//ASSERT(m_pTcpDrv);
}

//CCommServiceCode  CCngpService::m_CodeTool;
CCngpService::CCngpService(CConnPoint *pAccount, DWORD dwCodeType, DWORD dwDrvType, LPCTSTR pDrvParam, 
...
void CCngpService::SendMsgToIF(LPVOID pMsg, DWORD &dwSequence, int &nStatus, int PriFlag)
...
//***SMGW40-04, 2004-12-19, jdz, modi begin***//
int CCngpService::SendMsgToIF(LPVOID pMsg, int PriFlag, int& Errcode)
//***SMGW40-04, 2004-12-19, jdz, modi end***//
{
	ASSERT(pMsg);
...
void CCngpService::OnShakeHandAck(LPVOID pMsg)
{
    ASSERT(0); //不再处理此信息
    return;
}
...
int CCngpService::OnRead(LPVOID pOrgBuff,int nOrgSize)
{
    char* pDestBuff = NULL;
	...
	case DECODE_SMPP_HEADER_ERROR:	//结构头有错,在解码中设置。
		ASSERT(0);
		OnError(Ret);
		break;
...
void CCngpService::OnResMsg(LPVOID pMsg)
{
    ASSERT(0); //不再处理此信息
    return;
}
...
BOOL CCngpService::InitInterface(DWORD dwCodeProtocol, DWORD dwDriverProtocol)
{
...
BOOL CCngpService::Connect(DWORD dwDriverProtocol, LPCTSTR pDriverParam)
{
    HANDLE hDrvObject;
    int iID;
...
int CCngpService::BuildLink()
{
    ...
    */
    ...
    //Sleep(40); //等待服务端生成接口
    /*
    sStr.Format("SMCIF Next To Login %s %s", m_sRemoteAccount, m_sRemotePassword);
...
BOOL CCngpService::GetValidString(LPCTSTR pOrgStr, int& iOrgSize, LPTSTR pBuff, int& iBuffSize)
{
	int i = 0;
...
int CCngpService::Login(LPCTSTR pInterface, LPCTSTR pPassword)
{
    ...
    Bind.ulBinderType = this->GetSendOrRecv()-1;   //正常情况
	// 对亚信网关的特殊处理
    //填入接口类型
    /*
    UC YxRecvSend=0;
...
void CCngpService::OnEnquireLink(PTRMSG pMsg)
{
...
void CCngpService::OnBindAck(PTRMSG pMsg)
{
	//changed by wujun
	CNGP_Message_header *pHead = (CNGP_Message_header *)pMsg;
    Cngp_Connect_Resp * pConnectResp = (Cngp_Connect_Resp* )(pMsg+sizeof(smpp_Message_header));
	//EI_BIND_ACK *pBindAck = (EI_BIND_ACK *)pMsg;
	if( (pHead->Command_Status) != CMD_STATUS_SUCCESS)
	{
        CString sStr;
        sStr.Format("主动连接失败，错误码为%d", \
            pHead->Command_Status);
		//***SMGW35-13, 2004-10-12, jdz, modi begin***//
        GetSmsApp()->WriteTestRecord((LPCSTR)sStr, 0);
		//***SMGW35-13, 2004-10-12, jdz, modi begin***//
        SetState(IF_BROKEN);
...
*******************************************************************************/
BOOL CCngpService::OnDeliverMsg(PTRMSG pBuffer, int iDrv)
{
...
void CCngpService::OnEnquireLinkAck(LPVOID pRevBuff)
{
    m_ShakeCount = m_iMaxShakeHandCount;
	//add by wj
	m_ShakeTick = ::GetTickCount();
...
void CCngpService::DoDeliverACK(DWORD dwHeadSequence, ULONG sm_ID, unsigned char Status, unsigned char CS, unsigned char FCS)
{
    ASSERT(0); //不由此处应答
    return;
}

void CCngpService::SendShakeHandToIF(LPVOID pMsg)
{
...
int CCngpService::SetCodeType(int iIFType)
{
	if( NULL != m_pCodeTool)
		delete m_pCodeTool;
	m_pCodeTool = NULL;
...
BOOL CCngpService::MakeDecodeErrorResp(LPCSTR pOrgBuff, \
									   int iOrgSize, char *pDestBuff, int &iDestSize, \
									   int iErrorCode)
{
    if (GetCodeType() == CODE_TYPE_CNGP)
    {
...
DWORD CCngpService::GetSequenceID()
{
