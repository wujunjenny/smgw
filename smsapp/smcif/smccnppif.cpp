...
CSmcCNPPIF::CSmcCNPPIF(HANDLE hDrvObject, UL Type,UL InterfaceID) \
...
CSmcCNPPIF::CSmcCNPPIF(CConnPoint *pAccount, DWORD dwCodeType, DWORD dwDrvType, LPCTSTR pDrvParam, 
...
void CSmcCNPPIF::SendMsgToIF(LPVOID pMsg,DWORD &dwSequence,int &nStatus, int PriFlag)
//***SMGW40-04, 2004-12-19, jdz, modi begin***//
{
	ASSERT(pMsg);
...
//***SMGW40-04, 2004-12-19, jdz, modi begin***//
int CSmcCNPPIF::SendMsgToIF(LPVOID pMsg, int PriFlag, int& ErrCode)
//***SMGW40-04, 2004-12-19, jdz, modi end***//
{
	ASSERT(pMsg);
...
void CSmcCNPPIF::OnShakeHandAck(LPVOID pMsg)
{
    ASSERT(0); //不再处理此信息
    return;
}
...
int CSmcCNPPIF::OnRead(LPVOID pOrgBuff,int nOrgSize)
{
...
void CSmcCNPPIF::OnResMsg(LPVOID pMsg)
{
    ASSERT(0); //不再处理此信息
    return;
}
...
BOOL CSmcCNPPIF::InitInterface(DWORD dwCodeProtocol, DWORD dwDriverProtocol)
{
	...
	case CODE_TYPE_CNPP:
        ...
        m_pCodeTool = NULL;
//changed by wujun
		m_pCodeTool = new CCnppCodeAndDecode;
		break;
	case CODE_TYPE_SMAP:
        if( NULL != m_pCodeTool)
            delete m_pCodeTool;
...
BOOL CSmcCNPPIF::Connect(DWORD dwDriverProtocol, LPCTSTR pDriverParam)
{
    HANDLE hDrvObject;
    int iID;
...
BOOL CSmcCNPPIF::GetValidString(LPCTSTR pOrgStr, int& iOrgSize, LPTSTR pBuff, int& iBuffSize)
{
	int i = 0;
...
int CSmcCNPPIF::Login(LPCTSTR pInterface, LPCTSTR pPassword)
{
...
void CSmcCNPPIF::OnEnquireLink(PTRMSG pMsg)
{
...
void CSmcCNPPIF::OnBindAck(PTRMSG pMsg)
{
//changed by wujun
    smpp_Message_header * pSmppHeader = (smpp_Message_header* )pMsg;
...
*******************************************************************************/
BOOL CSmcCNPPIF::OnDeliverMsg(PTRMSG pBuffer, int iDrv)
{
...
void CSmcCNPPIF::OnEnquireLinkAck(LPVOID pRevBuff)
{
    m_ShakeCount = m_iMaxShakeHandCount;
}

void CSmcCNPPIF::DoDeliverACK(DWORD dwHeadSequence, ULONG sm_ID, unsigned char Status, unsigned char CS, unsigned char FCS)
{
    ASSERT(0); //不由此处应答
    return;
/*
...
void CSmcCNPPIF::SendShakeHandToIF(LPVOID pMsg)
{
...
int CSmcCNPPIF::OnAddWaitAck(PTRMSG pMsg)
{
    ASSERT(0); //不再处理此信息
    return FALSE;
...
BOOL CSmcCNPPIF::OnSubmitAck(PTRMSG pMsg)
{
//    POSITION pos;
...
void CSmcCNPPIF::RemoveWaitAck(UL ulSequence)
{
    ASSERT(0); //接口不再管理对应
    return;
	CSmcIFWaitAck *pWaitAck = NULL;
...
void CSmcCNPPIF::RefreshWaitAck()
{
    ASSERT(0); //接口不再管理对应
    return ;
	POSITION pos;
...
int CSmcCNPPIF::SetCodeType(int iIFType)
{
    switch(iIFType)
	{
        case CODE_TYPE_SERVICE:
...
BOOL CSmcCNPPIF::MakeDecodeErrorResp(LPCSTR pOrgBuff, \
             int iOrgSize, char *pDestBuff, int &iDestSize, \
             int iErrorCode)
{
    if (GetCodeType() == CODE_TYPE_CNPP)
    {
...
DWORD CSmcCNPPIF::GetSequenceID()
{
