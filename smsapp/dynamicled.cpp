...
void CDynamicLED::OnTimer(UINT nIDEvent) 
{
	...
	if(m_bBright==TRUE)
	{
		...
		oldPen = dc.SelectObject(&m_PenBright);
		oldBrush = dc.SelectObject(&m_BrushBright);
	...
	else
	{
		...
		oldPen = dc.SelectObject(&m_PenDark);
		oldBrush = dc.SelectObject(&m_BrushDark);
    ...
    CPen* oldThickPen;CBrush* pBrush;
	if(m_nShape==ID_SHAPE_ROUND)
	{
		...
		oldThickPen = dc.SelectObject(&Pen);
		
		dc.Ellipse(rcClient);
        dc.SelectObject(oldThickPen);
