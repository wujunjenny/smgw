...
CSmgpService::CSmgpService(HANDLE hDrvObject, UL Type,UL InterfaceID) \
...
CSmgpService::CSmgpService(CConnPoint *pAccount, DWORD dwCodeType, DWORD dwDrvType, LPCTSTR pDrvParam, 
...
void CSmgpService::SendMsgToIF(LPVOID pMsg,DWORD &dwSequence,int &nStatus, int PriFlag)
...
//***SMGW40-04, 2004-12-19, jdz, modi begin***//
int CSmgpService::SendMsgToIF(LPVOID pMsg, int PriFlag, int& ErrCode)
//***SMGW40-04, 2004-12-19, jdz, modi begin***//
{
	ASSERT(pMsg);
...
void CSmgpService::OnShakeHandAck(LPVOID pMsg)
{
    ASSERT(0); //不再处理此信息
    return;
}
...
int CSmgpService::OnRead(LPVOID pOrgBuff,int nOrgSize)
{
    char *pDestBuff = NULL;
...
void CSmgpService::OnResMsg(LPVOID pMsg)
{
    ASSERT(0); //不再处理此信息
    return;
}
...
BOOL CSmgpService::InitInterface(DWORD dwCodeProtocol, DWORD dwDriverProtocol)
{
...
BOOL CSmgpService::Connect(DWORD dwDriverProtocol, LPCTSTR pDriverParam)
{
    HANDLE hDrvObject;
    int iID;
...
BOOL CSmgpService::GetValidString(LPCTSTR pOrgStr, int& iOrgSize, LPTSTR pBuff, int& iBuffSize)
{
	int i = 0;
...
int CSmgpService::Login(LPCTSTR pInterface, LPCTSTR pPassword)
{
...
// 对亚信网关的特殊处理
	...
	if (this->GetAccount()->GetServiceType()==SERVICE_SMC)
       YxRecvSend=0;
	else
       YxRecvSend=1;
...
void CSmgpService::OnEnquireLink(PTRMSG pMsg)
{
...
void CSmgpService::OnBindAck(PTRMSG pMsg)
{
//changed by wujun
    Smgp_Connect_Resp * pConnectResp = (Smgp_Connect_Resp* )(pMsg+sizeof(smpp_Message_header));
...
*******************************************************************************/
BOOL CSmgpService::OnDeliverMsg(PTRMSG pBuffer, int iDrv)
{
...
void CSmgpService::OnEnquireLinkAck(LPVOID pRevBuff)
{
    m_ShakeCount = m_iMaxShakeHandCount;
	//add by wj
	m_ShakeTick = ::GetTickCount();
...
void CSmgpService::DoDeliverACK(DWORD dwHeadSequence, ULONG sm_ID, unsigned char Status, unsigned char CS, unsigned char FCS)
{
    ASSERT(0); //不由此处应答
    return;
}

void CSmgpService::SendShakeHandToIF(LPVOID pMsg)
{
...
int CSmgpService::SetCodeType(int iIFType)
{
     if( NULL != m_pCodeTool)
         delete m_pCodeTool;
     m_pCodeTool = NULL;
...
BOOL CSmgpService::MakeDecodeErrorResp(LPCSTR pOrgBuff, \
             int iOrgSize, char *pDestBuff, int &iDestSize, \
             int iErrorCode)
{
    if (GetCodeType() == CODE_TYPE_SMGP)
    {
...
DWORD CSmgpService::GetSequenceID()
{
...
/*
void  CSmgpService::SendShakeHand()
{


  Message_header Hand;
  Hand.Command_ID=Smgp_ACTIVE_TEST;
...
*/
//void  CSmgpService::OnShakeHand(void)
//{
